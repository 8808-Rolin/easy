...
## 1.2 编写目的
本文档的编制是为了让用户和软件开发者双方对开发软件的UI(User Interface 用户交互)初始规定有一个共同理解，定义所要开发的“Easy社团一体系统”的用户界面布局、风格等设计目标，是整个项目中软件产品开发设计与实现的重要根据，也是软件产品的测试和验收的依据
## 1.2 开发背景
- 系统名称： **Easy社团**
...
## 4.3 论坛界面设计
### 4.3.1 结构布局
论坛页面的界面设计采用上下布局，上方是一些信息数据展示，下方是论坛的帖子区
而信息数据展示部分也分为两部分，分别是个人信息展示和论坛功能性数据展示。帖子区分为展示帖子和发表帖子区域
### 4.3.2 公共论坛界面原型图
